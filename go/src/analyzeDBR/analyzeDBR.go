...
*/
func getParams(configFile *string, region *string, date *string, bucket *string, account *string) error {
	...
	regexDate := regexp.MustCompile(`^\d{6}$`)
...
func main() {
	...
	if err := getParams(&configFile, &region, &date, &bucket, &account); err != nil {
		log.Fatal(err)
	}
